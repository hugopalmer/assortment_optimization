...
def run_GDT(Inventories, Proba_product, ITERATIONS_MAX=10, eps_stop=0):
    ...
    A=np.full((nb_prod, nb_prod,nb_asst), fill_value=0, dtype=np.float64) #first component is the number of columns generated at the begining
    for k in range(nb_prod):
        sigma_GDT[k,k]=0
        A[k,:,:] = sigma2a(sigma_GDT[k,:], Inventories)

    #first call to restricted master
    ...
    for w in range(ITERATIONS_MAX):
        ...
        [new_sigma_GDT, new_A, new_rc] = lowest_reduced_cost(set_k_possible, sigma_GDT, nb_prod, alpha_found, nu_found, Inventories, 20)
...
def lowest_reduced_cost(set_k_possible, sigma_GDT, nb_prod, alpha_found, nu_found, assortments, n_new_branches=100):
    ...
    sort = np.argsort(new_rc)[:min(n_new_branches, len(new_rc))]#we take the n_new_branches smallest rc (exception if n_new_branches is > len(rc) )
...
def choose_n(lambda_found, n):
    ...
    return np.random.choice(len(lambda_found), size=min(n,len(np.nonzero(lambda_found)[0])), replace=False, p=lambda_found)
...
def sigma2a(sigma, assortments):
    ...
    try:
        ...
        assts_not_yet_processed = np.arange(nb_asst)[np.invert(assortments[:,preferred_prod]) & np.invert(assortments[:,scd_preferred_prod])]#absence of preferred product and presence of second preferred product
    except:
        a=1#useless
    ...
    asst_without_choice = np.where(assortments[:,np.where(sigma==nb_prod-1)[0]].sum(axis=1)==assortments.sum(axis=1))[0]#True when  the two previous numbers are equal: ie we know that no product preferred is in the asst!
